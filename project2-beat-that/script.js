...
var selectBA = function () {
  number = Number(`${arrayRolls[1]}` + `${arrayRolls[0]}`);
  return number;
};
...
var getLeaderboard = function () {
  var myOutputValue = [`Player2 : ${playerTwoScore}`, ` Player1 : ${playerOneScore}`];
  if (playerOneScore >= playerTwoScore) {
    myOutputValue = [`Player1 : ${playerOneScore}`, ` Player2 : ${playerTwoScore}`];
  }
  return myOutputValue;
};
...
var roll = function () {
  ...
  myOutputValue = `Player ${currentPlayer}, your two dice rolls are:<br><br> 
  A -- ${arrayRolls[0]}<br> B -- ${arrayRolls[1]}. <br><br> Please pick the order of the number AB or BA `;
...
var variablePlayers = function (input) {
  var myOutputValue = "";
  if (L2GameMode == L2_CHOOSE_NUM_DICE) {
    myOutputValue = chooseNumOfPlayers(input);
  } else if (L2GameMode == L2_ROLL_DICE) {
    myOutputValue = rollMultiPlayer(input);
  } else if (L2GameMode == L2_DECIDE_WINNER) {
    myOutputValue = decideWinnerMulti(input);
  }
  return myOutputValue;
};
var chooseNumOfPlayers = function (input) {
  numOfPlayers = input;
  var myOutputValue = `You have choosen to play with ${input} players ⛹🏻‍♀️⛹🏻‍♂️. Hit Submit to start rolling!`;
  console.log("numofPlayers", numOfPlayers);
  L2GameMode = L2_ROLL_DICE;
  return myOutputValue;
};

var rollMultiPlayer = function () {
  var myOutputValue = "";
  var counterA = 0;
  var line1 = "";
  var line2 = "";
  ...
  line2 = `Computer picked ${bestNumber} for you.`;
  arrayForNums.push(bestNumber);
...
var pushPlayerNum = function (input) {
  var number = Number(input);
  arrayForNums.push(number);
};

var decideWinnerMulti = function () {
  var myOutputValue = "None";
  playerOneScore = getScores(1);
  playerTwoScore = getScores(2);
  var leaderBoard = getLeaderboard();
